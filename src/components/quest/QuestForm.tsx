...
export const QuestForm: React.FC<QuestFormProps> = ({ onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.title.trim()) {
      ...
      showToast('⚠️ Quest name cannot be empty! Give your quest a name.', 'error');
    } else if (formData.title.trim().length < 3) {
      newErrors.title = 'Quest name must be at least 3 characters';
      showToast('⚠️ Quest name is too short! Minimum 3 characters required.', 'warning');
    } else if (formData.title.length > 50) {
      newErrors.title = 'Quest name too long! Max 50 characters';
      showToast('⚠️ Quest name is too long! Maximum 50 characters allowed.', 'warning');
    }
    ...
    setErrors({});
    addTask(formData);
    showToast('✅ Quest created successfully! Adventure awaits!', 'success');
  ...
  const handleAddTag = () => {
    ...
    if (formData.tags.includes(trimmedTag)) {
      showToast('⚠️ This tag already exists!', 'warning');
      return;
    }
    
    if (formData.tags.length >= 5) {
      setErrors({ ...errors, tags: 'Maximum 5 tags allowed' });
      showToast('⚠️ Maximum 5 tags allowed per quest!', 'warning');
      return;
    }
    ...
    <>
      ...
      {toast && (
        <motion.div
          ...
          className={`fixed bottom-6 left-1/2 z-[100] px-6 py-3 rounded-lg shadow-2xl 
                   border-2 font-bold text-sm max-w-md w-full text-center
                   ${toast.type === 'error' 
